...
const collect = (dom: DOMUtils, rngOrNode: Range | Node, rootNode: Element, propName: FontProp): string[] => {
  ...
  const process = (nodes: Node[], limit: Node, getComputed: boolean) => {
    Arr.each(nodes, (node) => {
      const parents = dom.getParents(node, '*[style],font', limit);
      Arr.findMap(parents, (parent) => getSpecifiedFontProp(propName, SugarElement.fromDom(parent)))
        .orThunk(() => {
...
const getFontInfo = (propName: FontProp, normalise: (str: string) => string) => (rootNode: Element, rngOrNode: Range | Node) => {
  const fontProps = Arr.map(collect(DOMUtils.DOM, rngOrNode, rootNode, propName), normalise);
